CephX 是什么？
CephX 理解起来很简单，就是整个 Ceph 系统的用户名/密码，而这个用户不单单指我们平时在终端敲 ceph -s 而生成的 client，在这套认证系统中，还有一个特殊的用户群体，那就是 MON/OSD/MDS，也就是说，Monitor， OSD， MDS 也都需要一对账号密码来登陆 Ceph 系统。
...
如果不存在这四个文件，或者在这四个文件里面均没有保存用户 client.admin 的秘钥，那么就会报错：ERROR: missing keyring。也就是说，用户client.admin 登陆 Ceph 系统失败！
...
caps osd = "allow *"
的确，内容不一样，这时候用 ceph auth get osd.0 得到的key值替换秘钥文件keyring的key部分即可正常启动 OSD。
...
报错信息很好理解，操作不被允许，也就是认证未通过，由于这里我们使用的是默认的 client.admin 用户和它的秘钥，说明秘钥内容和 Ceph 集群记录的不一致，也就是说 /etc/ceph/ceph.client.admin.keyring 内容很可能是之前集群留下的，或者是记录了错误的秘钥，这时，只需要使用 mon. 用户来执行 ceph auth list就可以查看到正确的秘钥内容：
...
w 权限
w 权限比较有趣，必须配合 r 权限才能有效力，否则，单独 w 权限执行指令时，是会一直 access denied的。所以我们在测试 w 权限时，需要附加上 r 权限才行：
...
由于这里可以执行的指令实在太多了，我仅仅对 r, w权限做一个简单的总结：

r : 读取集群各个组件(MON/OSD/MDS/CRUSH/PG)的状态，但是不能修改。
rw : 读取并可以修改集群的各个组件的状态，可以执行对组件的各个动作指令。
注意：
...
>
在指令指定 --name后，Ceph 就会到 --keyring 后面的文件中找寻对应的用户名section下的秘钥。

profile osd 权限
官方的解释是：给用户以一个 OSD 的身份 连接到 其他 OSD/MON 的权限。给 OSD 授予这个权限，使得 OSD 能够处理副本心跳和状态汇报。暂时没找到比较通俗的理解~

OSD
相比于 MON 的各个权限，OSD 的 rw 比较简单理解一下，r 权限就是读取对象的权限， w 权限就是写对象的权限，x 权限比较有趣，可以调用任何 class-read/class-write的权限，这里引用 Ceph CookBook 上的一段话来介绍 class-read/class-write：
...
举个例子，你可以实现一些自定义的方法，通过调用这些方法，可以读写具有某一类特征的对象，比如都是以rbd_data开头的对象。目前只有调用librados层才能使用自定义class方法，而上层RBD，RGW之类的是不能使用的。

* 权限除了包含了 rwx，还包含了ceph tell osd.* 这类admin指令的权限。

官网的这个页面(http://docs.ceph.com/docs/kraken/man/8/ceph-authtool/)比较好的介绍了几个实例，这里简单介绍下一个比较长的指令的意义：

osd = "allow class-read object_prefix rbd_children, allow pool templates r class-read, allow pool vms rwx"
第一个权限：object_prefix 是一个类方法，而这个方法的作用是给予所有以rbd_children为名开头的对象的class-read也就是读权限。只能读，不能写。

第二个权限：给予池 templates的读权限，可以执行class-read方法的权限。也就是说，客户除了可以读这个池的对象外，还能自己实现形如obejct_prefix这种系统自带的类方法来读取对象，比如读取具备某一类特征的对象。
...
在管理秘钥方面，Ceph 做了一个比较有趣的设定：所有除了 mon. 用户的账户密码都保存在 MON 的数据库leveldb中，但是 mon. 用户的信息并没有保存在数据库里，而是在 MON 启动时读取 MON 目录下的 keyring 文件得到的。所以，我们甚至可以随便伪造一个keyring，放置到 MON 目录下去。然后同步到各个 MON 节点，然后重启三个 MON。 这时候，MON就拿着人造的keyring启动生效了。有了 mon.用户的账户密码，我们很容易的可以使用 ceph auth list --name mon. --keyring /var/lib/ceph/mon/ceph-$hostname/keyring 指令来得到所有的秘钥内容！

等等，如果真的删干净了/etc/ceph/目录的话， 上面的这个指令是不能执行的，因为没有/etc/ceph/ceph.conf去指定集群，这时候，我们可以从任意一个OSD目录下的 /var/lib/ceph/osd/ceph-0/ceph_fsid 得到集群的fsid，MON 的IP信息也很容易恢复。简单得重构了 ceph.conf后，使用mon. 的用户密码就可以得到正确的 ceph auth list的信息啦~。
...
原来是,zabbix_agent 对 /etc/ceph/ceph.client.admin.keyring 没有读权限，将秘钥 chmod a+r之后，就能正常读取数据了。
