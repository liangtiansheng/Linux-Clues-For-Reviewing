...
/WEB-INF  此目录为私有资源目录，其内部的所有文件和子目录均不能被公开访问；包含着此  Web应用程序的配置文件web.xml（程序结构描述符文件）通常放置于此目录；
...
ProxyPreserveHost {On|Off}：如果启用此功能，代理会将用户请求报文中的Host:行一并发送给后端的服务器，请求的就是指定主机名的tomcat host，若不启用，请求时直接用proxypass的IP 地址，显示的是后端tomcat的默认host页面。如果想在反向代理中支持虚拟主机，则需要开启此项，否则就无需打开此功能。
