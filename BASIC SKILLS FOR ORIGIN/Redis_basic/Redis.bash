...
Redis:
	KV cache and store
		in-memory内存中操作 1000000个key->value 大约100M
		持久化
		主从(借助于sentinel实现一定意义上的HA)
		Clustering(分布式)
	单线程，但是cpu不会成为瓶颈，键值本身很轻量，每秒500000个请求响应
	...
	Redis的持久化：
		RDB和AOF
			...
			AOF: Append Only File记录每一次写操作至指定的文件尾部实现持久化；当redis重启时，可通过重新执行文件中的命令在内存重建数据库；
				重写过程：避免文件会越来越大，按指定好的规则周期重写
