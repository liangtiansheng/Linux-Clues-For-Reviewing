...
Nginx特性：
	libevent: 高性能的网络库
		epoll():就是signal driven模型
	模块化设计、较好的扩展性、不能动态加载，目前只能静态编绎
	高可靠
		master --> worker
			进程管理机制优秀
				一个主控进程
					读取配置、启动关闭sock、生成关闭子进程，平滑升级等
				多个子进程
			低内存消耗
				10000个keep-alive模式下的connection，仅需要2.5MB的内存
			支持热部署
				不停机而更新配置文件、日志文件滚动、升级程序版本
	支持事件驱动机制、AIO、MMAP(内存映射机制)

	基本功能：
		静态资源的web服务器，能缓存打开的文件描述符：
		http、smtp、pop3协议的反向代理服务器
			Nginx反向代理：	缓存加速、安全控制、处理请求、为后端服务器减轻压力
		缓存加速、负载均衡；
		支持FastCGI(fpm,LNMP), uWSGI(Python)等；
		模块化(非DOS机制)、过滤器zip、SSI及图像的大小调整；
		支持SSL；
	扩展功能：
		基于名称和IP的虚拟主机；
		支持keepalive
		...
		支持速率限制，支持并发数限制
sendfile: 
	网络请求页面一般方式：client-->net_IO-->kernel-->userspace_app-->kernel-->disk_IO-->FILE-->kernel-->userspace_app-->kernel-->net_IO--client
	而sendfile是client-->net_IO-->kernel-->userspace_app-->kernel-->disk_IO-->FILE-->kernel-->net_IO-->client
	
	Nginx的基本架构：
		一个master进程，生成一个或多个worker进程；
		siganl driven：epoll(边缘触发)、kqueue, /dev/poll
			复用器：select, poll, rt signal
		支持sendfile，sendfile64
		支持AIO
		支持MMAP
	nginx的工作模式：非阻塞、事件驱动、由一个master进程生成多个worker线程，每个worker响应n个请求;
		worker*n即是总数，一般单机5万个了不解了，套接字才65536个
	...
	安装	
		...
		配置文件：
			...
			主配置段的指令：
				...
				事件相关的配置
					1、accept_mutex {off|on}
						master调度用户请求至各worker进程时使用的负载均衡锁；on表示能让多个worker轮流有序地响应新请求
					2、lock_file file;
						accept_mutex用到的锁文件路径
					3、user [epoll|rtsig|select|poll]
						用于指明使用的IO复用模型，建议让nginx自选
			...
			Nginx作为web服务器时使用的配置：
				...
				注意：与http相关的指令只能放置http、server、location、upstream、if上下文中，某些只能用在其中的某几个上下文中
				1、server{}
					定义虚拟主机
						http {
                            server {
				...
				5、location [ = | ~ | ~* | ^~ ] uri {...}
					location @name {...}
					功能：允许根据用户请求的URI来匹配定义的各Loction；匹配到时，此请求将会被相应的location块中的配置所处理
					比如，访问控制等
				...
				6、alias path;
					...
					http://www.ly.com/images/a.jpg <--- /vhosts/web1/images/a.jpg
					
					location /images/ {
						alias "/www/pictures";
					}
					http://www.ly.com/images/a.jpg <-- /www/picture/a.jpg
				...
				13、rewrite regex replacement flag;
					...
					http://www.ly.com/images/a/b/c/1.jpg --> /imgs/a/b/c/1.jpg
					
					flag:
					    last：一旦此rewrite规则重写完成后，就不再被后面其它的rewrite规则进行处理，而是由
							User Agent重新对重写后的URL再一次发起请求，并从头开始执行类似的过程
					    break：一旦此rewrite规则重写完成后，由Uer Agent对新的URL重新发起请求，且不再会被当前location内的任何rewrite规则所检查
							可以避免死循环
				...
				14、if
					...
					condition:
						(1) 变量名：
							变量值为空串，或者以"0"开始，则为false;其它的均为ture;
						(2)以变量为操作数构成的比较表达式
							可使用=, !=类似的比较操作符进行测试；
						(3)正则表达式的模式匹配操作
							~：区分大小写的模式匹配检查
				...
				15、防盗链
					location ~* \.(jpg|gif|jpeg|png)${
						valid_referer none blocked www.ly.com;
						if ($invalid_referer){
						   rewrite ^/ http://www.ly.com/403.html;
						}

					}
				16、定制访问日志格式
					log_format main	log_format  main  '$remote_addr - $remote_user [$time_local] "$request" '
...
nginx反向代理：七层代理，在nginx用户空间是要拆开client报文，可以分析资源
	ngx_http_proxy_module模块
		server {
		    listen
		    server_name
		    location / {
			proxy_pass http://192.168.154.111:80;
		    }
		 }
